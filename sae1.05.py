...
fic = open("Tableau.csv", "w")  # test est le fichier d'arrivée des extractions
# intitulé de mes colonnes
evenement = "DATE ; SOURCE ; PORT ; DESTINATION ; FLAG ; SEQ ; ACK ; WIN ; OPTIONS ; LENGTH"
fic.write(evenement + "\n")  # écriture de mes titres dans le tableur
# définir une variable avec le caractère ":" (qui nous sera utile pour la suite)
characters = ":"
for event in ress:
    # évenement qui commence par "11:42" (ils commencent tous par 11:42)
    if event.startswith('11:42'):
        ...
        if len(texte) > 1:  # s'il y a plus de une partie à partir du crochet
            # on prend après le premier crochet et on coupe au deuxième crochet
            flag1 = texte[1].split("]")
            # pourquoi 0 ? Car on prend la partie de gauche du deuxième crochet (ce qu'on recherche)
            flag = flag1[0]
        # seq
        ...
        if len(texte) > 1:  # s'il y a plus de 1 partie à partir du crochet
            if texte[1].startswith(" seq"):  # Si le texte [1] commence par " seq"
                # on coupe à l'espace et on prend le texte juste après
                seq1 = texte[1].split(" ")
                # On a 2 parties entre le split ',' et ce que je recherche
                seq = seq1[2]
        # ack
        if len(texte) > 2:
        ...
        if len(texte) > 3:  # si le nombre de partie est supérieur à 3
            ...
            if texte[3].startswith(" win"):  # Si le texte [3] commence par " win"
                # on coupe à l'espace et on prend le texte juste après
                win1 = texte[3].split(" ")
                # On prend le texte après l'espace (la partie qu'on retrouvera dans le tableau)
                win = win1[2]
        ...
        if len(texte) > 2:
            ...
            # pourquoi 0 ? Car on prend la partie à gauche du deuxième crochet "]"
            options = options1[0]
        ...
        if len(texte) > 2:  # vérifier le nombre de partie (split au crochet)
            ...
            # On veut bien le "2" pour avoir que le nombre (il y a une partie avant l'espace + une autre après).
            length = length1[2]
        # length (sans option)
        texte = event.split(",")
        if len(texte) > 3:
            # Si ça commence par " length" et on recherche dans le texte [3]
            if texte[3].startswith(" length"):
                length1 = texte[3].split(" ")  # on coupe à l'espace
                # On veut bien le "2" pour avoir que le nombre (texte [1] avant l'espace c'est le mot "length").
                length = length1[2]
                # remplacement du "characters" en " " (pour éviter que le tableur écrit sous forme de date)
                length = length.replace(characters, "")
        # dès que le programme arrive à la dernière ligne du fichier texte
        if event.startswith("11:42:55.536521"):
            prog = 0  # il ne fait plus de tour, il s'arrete
        evenement = heure1+";"+nomip + ";" + port + ";" + nomip2 + ";"+flag + \
            ";" + seq + ";" + ack + ";" + win + ";" + options + ";" + length
